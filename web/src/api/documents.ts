...
import type { DocumentResponse, DocumentVersionResponse } from '../types/index.ts';
...
export const getDocumentVersions = (id: string): Promise<DocumentVersionResponse[]> =>
...
export const uploadDocumentVersion = async (
  ...
  const token = await getToken();
  const baseUrl = import.meta.env.VITE_API_BASE_URL || '/api';
  const url = `${baseUrl}/documents/${encodeURIComponent(id)}/versions`;
  const response = await fetch(url, {
    method: 'POST',
    headers: {
      'Content-Type': file.type || 'application/octet-stream',
...
export const downloadDocumentVersion = async (
  ...
  const baseUrl = import.meta.env.VITE_API_BASE_URL || '/api';
  const response = await fetch(
    `${baseUrl}/documents/${encodeURIComponent(id)}/versions/${version}/download`,
    {
      headers: token ? { Authorization: `Bearer ${token}` } : {},
    },
  );
