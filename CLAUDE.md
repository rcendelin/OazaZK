...
- **Component-split payments + per-house saldo.** `AdvancePayment` carries a 3-way split `WaterAmount`/`ElectricityAmount`/`CommonAmount` (+ `Amount`=sum), a `PaymentType` (`Advance`|`Doplatek`) and a `Note`. A **doplatek** is an ad-hoc, repeatable payment (PK=houseId, RowKey `D-{invertedTicks}-{guid}`); a regular advance stays one-per-house-per-month (RowKey `YYYY-MM`). `Settlement` is extended with `ElectricityCharge`/`ElectricityAdvances`/`CommonCharge`/`CommonAdvances`; `TotalAdvances` is now WATER advances only (was a lump). **Saldo is period-based** (charges locked per period — closed=snapshot, open=live preview — paid recomputed live from the ledger so post-close doplatky count; payments outside any period = "Nezařazené platby" credit). Endpoints: `POST /advances/doplatek`, `DELETE /advances/{houseId}/{rowKey}`, `GET /advances/saldo` (member=own house, admin/accountant=all). UI: `web/src/pages/SaldoPage.tsx` (route `/saldo`). See `CalculateHouseSaldoUseCase`.
